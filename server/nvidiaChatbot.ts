...
import { buildPutikKnowledgeContext } from "./chatbotKnowledge";
...
function detectSpecificCase(input: string) {
  ...
  return {
    isSpecific: specificSignals.some((signal) => lower.includes(signal)),
    wantsHuman: handoffSignals.some((signal) => lower.includes(signal)),
  };
}

function chooseRoute(message: string): { route: ChatbotRoute; handoffReason: string | null } {
  const { isSpecific, wantsHuman } = detectSpecificCase(message);

  if (wantsHuman) {
    return {
      route: "live_chat_handoff",
      handoffReason: "Pengguna secara eksplisit meminta bantuan petugas atau live chat.",
    };
  }
  ...
  if (isSpecific) {
    return {
      route: "case_triage",
      handoffReason: "Pertanyaan terindikasi terkait kasus atau tindak lanjut spesifik.",
    };
  }
...
async function callModel(provider: ProviderConfig, modelId: string, messages: ChatMessage[]) {
  ...
  if (typeof content !== "string" || !content.trim()) {
    throw new Error(`${provider.name}/${modelId} returned empty content`);
  }
  return content.trim();
}
...
export async function generatePutikChatReply(userMessage: string): Promise<ChatbotResponse> {
  ...
  const knowledge = buildPutikKnowledgeContext();
