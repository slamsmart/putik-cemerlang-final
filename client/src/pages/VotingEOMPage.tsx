...
import {
  Award, Vote, CalendarDays, Users, ShieldCheck, Trophy, Medal, Star, Globe, Mail, Phone, CheckCircle2, Lock
} from "lucide-react";
...
export default function VotingEOMPage() {
  ...
  // UUID unik per browser — tersimpan di localStorage agar persisten
  // Mengatasi masalah shared IP (seluruh kantor/WiFi sama IP-nya)
  const [voterId] = useState<string>(() => {
    const KEY = "eom_voter_id";
  ...
  // Hanya cek by device UUID (localStorage) untuk menentukan apakah user sudah vote
  // IP TIDAK digunakan untuk memblokir — shared WiFi kantor harus bisa vote tiap device
  const hasVotedByDevice = useConvexQuery(
    api.votingEom.checkVotedByDevice,
    { voterId, periode }
  );
  const hasVoted = hasVotedByDevice;
  ...
  const confirmVote = async () => {
    ...
    // Fix: jangan silent fail — tunjukkan error jika IP masih loading
    ...
    try {
      await castVote({
        ...
        ipAddress: clientIp,
        voterId,           // kirim UUID device untuk deduplication per-perangkat
        periode,
      });
      setVotingDone(true);
      ...
      <main>
            ...
            <div className="max-w-[1200px] mx-auto px-4 md:px-6 relative z-10">
              <div className="text-center mb-16">
                <h1 className="text-4xl sm:text-5xl md:text-7xl font-bold mb-8 tracking-tight font-['Public_Sans',Helvetica]">
                  Pegawai Teladan Triwulan I
                </h1>
                <p className="text-lg md:text-xl text-white/80 max-w-2xl mx-auto leading-relaxed">
                  Apresiasi dedikasi dan kinerja terbaik rekan kerja Anda. Berikan suara untuk kandidat yang paling
                  mencerminkan nilai integritas Cabang Dinas Kelautan dan Perikanan Kabupaten Malang.
                </p>
              </div>
        ...
        <section className="pt-32 pb-8">
          <div className="max-w-[1200px] mx-auto px-4 md:px-6">
            <div className="bg-white border border-slate-200 p-6 rounded-2xl flex items-center gap-5 shadow-sm max-w-3xl mx-auto">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-slate-600">
                  Setiap perangkat hanya diperbolehkan memberikan <strong>satu suara</strong> per periode.
                  Voting dibatasi untuk <strong>{maxVotes} pemilih</strong> per periode.
                </p>
              </div>
              {/* Kuota mini badge */}
                ...
                <AnimatePresence>
                  {candidates.map((c) => (
                    <motion.div
                      ...
                      className="bg-white rounded-[2rem] p-4 shadow-sm border border-slate-200 group hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
                    >
                      <div className="aspect-[4/5] rounded-[1.5rem] overflow-hidden mb-6 relative bg-slate-100">
                        {c.imageUrl ? (
                          <img
                            alt={c.nama}
                      ...
                      <div className="px-4 pb-4">
                        ...
                        <span className="text-sm font-semibold text-[#fb8c00] uppercase tracking-widest block mb-6">
                          {c.seksi}
                        </span>
