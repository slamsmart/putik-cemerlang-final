...
export default function WhistleBlowingAdminPage() {
  const { toast } = useToast();
  const [viewItem, setViewItem] = useState<WBSItem | null>(null);

  const { data: list = [], isLoading } = useQuery<WBSItem[]>({
    queryKey: ["/api/whistle-blowing"],
    refetchInterval: 10000,
  });

  const updateStatusMut = useMutation({
    mutationFn: ({ id, status }: { id: string; status: string }) =>
      apiRequest("PATCH", `/api/whistle-blowing/${id}/status`, { status }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/whistle-blowing"] });
  ...
  const removeMut = useMutation({
    mutationFn: (id: string) => apiRequest("DELETE", `/api/whistle-blowing/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/whistle-blowing"] });
  ...
  return (
    <AdminLayout>
      <header className="mb-10 flex items-end justify-between">
        <div>
          <h1 className="text-base font-normal text-[#001e40] [font-family:'Public_Sans',Helvetica]">Whistle Blowing System</h1>
          <p className="mt-1 text-base text-[#5f5e5e] [font-family:'Inter',Helvetica]">Kelola laporan indikasi pelanggaran yang masuk secara rahasia.</p>
        </div>
      </header>

      <div className="mb-6 grid grid-cols-4 gap-4">
        {[
          { label: "Total Laporan", value: list.length, color: "text-[#001e40]" },
      ...
      <Card className="rounded-xl border border-[#c3c6d1] bg-white shadow-[0px_1px_2px_#0000000d]">
        <CardContent className="p-6">
          <div className="overflow-x-auto">
              ...
              <tbody>
                {list.map((item) => (
                  <tr key={item._id} className="border-b border-slate-50 hover:bg-slate-50">
                    <td className="py-3 pr-4 text-sm text-[#191c1e] whitespace-nowrap">
            ...
            {!isLoading && list.length === 0 && <div className="py-10 text-center text-sm text-slate-400">Belum ada laporan WBS.</div>}
          </div>
        </CardContent>
      </Card>
      ...
      <Dialog open={!!viewItem} onOpenChange={() => setViewItem(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-[#001e40]">Detail Laporan WBS</DialogTitle>
          </DialogHeader>
          {viewItem && (
